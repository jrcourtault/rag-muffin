...
class GetDocumentTest {
    ...
    void shouldReturnDocument_whenExists() throws Exception {
        mockMvc.perform(get("/api/workspaces/{workspaceId}/documents/{id}", WORKSPACE_A, INDEXED_DOC_ID)
                ...
                .andExpect(jsonPath("$.sizeBytes").value(125000))
                .andExpect(jsonPath("$.status").value("INDEXED"))
                .andExpect(jsonPath("$.chunkCount").value(15))
    ...
    @Test
    @Sql(scripts = {"/sql/clean.sql", "/sql/workspaces.sql", "/sql/users.sql", "/sql/documents.sql"})
    void shouldReturnDocument_whenPending() throws Exception {
        mockMvc.perform(get("/api/workspaces/{workspaceId}/documents/{id}", WORKSPACE_B, PENDING_DOC_ID)
                        .with(jwt(SUBJECT)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(PENDING_DOC_ID))
                .andExpect(jsonPath("$.fileName").value("Manuel parachute.pdf"))
                .andExpect(jsonPath("$.status").value("PENDING"))
                .andExpect(jsonPath("$.chunkCount").isEmpty());
    }

    @Test
    @Sql(scripts = {"/sql/clean.sql", "/sql/workspaces.sql", "/sql/users.sql"})
    void shouldReturn404_whenNotFound() throws Exception {
        mockMvc.perform(get("/api/workspaces/{workspaceId}/documents/{id}", WORKSPACE_A, UUID.randomUUID())
                        .with(jwt(SUBJECT)))
